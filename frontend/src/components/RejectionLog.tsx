  ...
  const filtered = useMemo(
    () =>
      stocks.filter((s) => {
        ...
        if (
          verdictF.length > 0 &&
          !verdictF.includes(s.valuation?.verdict ?? "")
        )
          return false;
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>
            Failed quant filter or LLM moat analysis. Click AI-analyzed rows to
            expand.
          </p>
        </div>
        <span
...
function RejectionRow({
  ...
  return (
    <>
      ...
      >
        <td
          className="px-4 py-3.5 font-mono font-bold text-sm"
          style={{ color: "var(--reject)" }}
        >
          {stock.ticker}
          <span
            className="block text-xs mt-0.5 tabular-nums font-normal"
            style={{ color: "var(--muted)" }}
          >
        ...
        <td
          className="px-4 py-3.5 text-right font-mono tabular-nums text-sm"
          style={{ color: "var(--text)" }}
        >
        ...
        <td className="px-4 py-3.5 text-right">
          {stock.valuation?.margin_of_safety != null ? (
            <MosBadge
              mos={stock.valuation.margin_of_safety}
              verdict={stock.valuation.verdict}
            />
          ) : (
        ...
        <td className="px-4 py-3.5">
          <VerdictChip
            pass={stock.passes_quant}
            label={stock.passes_quant ? "✓ PASS" : "✗ FAIL"}
          />
        </td>
        <td className="px-4 py-3.5">
          {stock.passes_quant ? (
        ...
        <td
          className="px-4 py-3.5 max-w-sm text-xs leading-relaxed"
          style={{ color: "var(--muted)" }}
        >
          {risk}
        </td>
...
function RejectionDetail({
  ...
  const moatColor: Record<string, string> = {
    WIDE: "var(--pass)",
    NARROW: "var(--warn)",
    NONE: "var(--reject)",
  };
    ...
    <div className="grid grid-cols-3 gap-6 pt-2">
            ...
            <div>
              ...
              {moat.red_flags.map((f, i) => (
                <div
                  ...
                  style={{ color: "var(--muted)" }}
                >
                  <span style={{ color: "var(--reject)", flexShrink: 0 }}>
                    −
                  </span>
                  {f}
                </div>
            ...
            <div>
              ...
              {moat.green_flags.map((f, i) => (
                <div
                  ...
                  style={{ color: "var(--muted)" }}
                >
                  <span style={{ color: "var(--pass)", flexShrink: 0 }}>+</span>
                  {f}
                </div>
              ))}
            </div>
      ...
      <div className="space-y-3 pt-0.5">
        <DetailMeta
          ...
          value={
            <span
              className="font-mono text-xs font-bold px-2 py-0.5 rounded-sm"
              style={{
                color: moatColor[moat.competitive_moat] ?? "var(--muted)",
                backgroundColor: `color-mix(in srgb, ${moatColor[moat.competitive_moat] ?? "var(--muted)"} 12%, transparent)`,
              }}
            >
        ...
        <DetailMeta
          ...
          value={
            <span className="text-xs" style={{ color: "var(--muted)" }}>
              {(stock.valuation_preset ?? "balanced").replace("_", "-")}
            </span>
          }
        />
      </div>
