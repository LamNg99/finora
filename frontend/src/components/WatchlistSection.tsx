  ...
  const filtered = useMemo(
    () =>
      stocks.filter((s) => {
        ...
        if (aiF.length > 0 && !aiF.includes(s.passes_moat ? "PASS" : "FAIL"))
          return false;
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>
            No FMP ratio data — LLM moat analysis only. Quant metrics not
            evaluated.
          </p>
        </div>
        <span
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3.5 text-right">
          {val?.margin_of_safety != null ? (
            <MosBadge mos={val.margin_of_safety} verdict={val.verdict} />
          ) : (
        ...
        <td className="px-4 py-3.5 text-center">
          {moat ? (
            <MoatBadge moat={moat.competitive_moat} />
          ) : (
            <span style={{ color: "var(--muted)" }}>—</span>
          )}
        </td>
    ...
    <div className="grid grid-cols-3 gap-6 pt-2">
      <div className="col-span-2">
        <p
          ...
          style={{ color: stock.passes_moat ? "var(--pass)" : "var(--reject)" }}
        >
