...
export default function AIInbox({ stocks }: { stocks: StockAnalysis[] }) {
  ...
  const filtered = useMemo(() => {
    return stocks.filter((s) => {
      ...
      if (moatF.length > 0 && !moatF.includes(s.moat?.competitive_moat ?? ""))
        return false;
      const verdictF = filters.verdict ?? [];
      if (verdictF.length > 0 && !verdictF.includes(s.valuation?.verdict ?? ""))
        return false;
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>
            Equities that cleared both the quantitative baseline and the LLM
            moat analysis.
          </p>
        </div>
        <span
          ...
          style={{
            backgroundColor: "color-mix(in srgb, var(--pass) 12%, transparent)",
            color: "var(--pass)",
          }}
        >
          {filtered.length !== stocks.length
            ? `${filtered.length} / ${stocks.length}`
            : stocks.length}{" "}
          Fortress {stocks.length === 1 ? "Asset" : "Assets"}
        </span>
      </div>
      ...
      {stocks.length === 0 ? (
        ...
        >
          <p className="text-sm" style={{ color: "var(--muted)" }}>
            No stocks have cleared every filter yet. Run a screen to populate
            this inbox.
          </p>
        </div>
      ) : (
    ...
    <>
      ...
      >
        <td className="px-4 py-3.5 align-top">
          ...
          <span
            className="block text-xs mt-0.5 font-mono tabular-nums"
            style={{ color: "var(--muted)" }}
          >
        ...
        <td
          className="px-4 py-3.5 align-top text-right font-mono tabular-nums"
          style={{ color: "var(--text)" }}
        >
        ...
        <td
          ...
          style={{ color: "var(--text)" }}
        >
          {stock.valuation?.avg_fair_value
            ? `$${stock.valuation.avg_fair_value.toFixed(2)}`
            : "—"}
        </td>
        <td className="px-4 py-3.5 align-top text-right">
          {stock.valuation?.margin_of_safety != null ? (
            <MosBadge valuation={stock.valuation} />
          ) : (
...
function ExpandedThesis({
  moat,
  stock,
}: {
  moat: NonNullable<StockAnalysis["moat"]>;
  stock: StockAnalysis;
}) {
  return (
    <div className="grid grid-cols-3 gap-6 pt-2">
      ...
      <div className="space-y-3 pt-0.5">
        ...
        <MetaPair
          label="P/E"
          value={stock.pe_ratio > 0 ? `${stock.pe_ratio.toFixed(1)}×` : "—"}
        />
        <MetaPair
          label="FCF / Share"
          value={
            stock.fcf_per_share > 0 ? `$${stock.fcf_per_share.toFixed(2)}` : "—"
          }
        />
        {stock.valuation && <ValuationBreakdown val={stock.valuation} />}
        <MetaPair
          label="Moat Sources"
          value={moat.moat_sources.join(", ") || "—"}
        />
        ...
        <MetaPair
          label="Quant Preset"
          value={stock.quant_preset ?? "default"}
        />
        <MetaPair
          label="Valuation Preset"
          value={(stock.valuation_preset ?? "balanced").replace("_", "-")}
        />
      </div>
    </div>
  );
...
function SignalCol({
  ...
  return (
    <div>
      <p
        className="text-xs font-mono font-semibold uppercase tracking-widest mb-1.5"
        style={{ color }}
      >
        {title}
      </p>
      {items.slice(0, 3).map((item, i) => (
        <div
          key={i}
          className="flex gap-1.5 text-xs mb-1"
          style={{ color: "var(--muted)" }}
        >
...
function SentimentBadge({ score }: { score: number }) {
  const color =
    score >= 8 ? "var(--pass)" : score >= 5 ? "var(--warn)" : "var(--reject)";
  return (
    <span
...
function ValuationBreakdown({ val }: { val: Valuation }) {
  const rows: [string, string][] = [
    ...
    ...(val.dcf_value !== undefined
      ? [
          [
            "DCF",
            `$${val.dcf_value.toFixed(2)}${val.dcf_cagr !== undefined ? ` (${val.dcf_cagr > 0 ? "+" : ""}${val.dcf_cagr}% CAGR)` : ""}`,
          ] as [string, string],
        ]
      : []),
    ...(val.pfcf_value !== undefined
      ? [["P/FCF ×20", `$${val.pfcf_value.toFixed(2)}`] as [string, string]]
      : []),
    ...(val.graham_value !== undefined
      ? [["Graham №", `$${val.graham_value.toFixed(2)}`] as [string, string]]
      : []),
  ];
  return (
    <div
      className="pt-1 pb-0.5 border-t border-b my-2"
