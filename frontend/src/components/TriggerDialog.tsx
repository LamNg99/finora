...
import {
  QUANT_PRESETS,
  VALUATION_PRESETS,
  DEFAULT_QUANT_PRESET,
  DEFAULT_VALUATION_PRESET,
} from "../data/presets";
...
export default function TriggerDialog({ onClose, onStarted }: Props) {
  ...
  const [valuationPreset, setValuationPreset] = useState(
    DEFAULT_VALUATION_PRESET,
  );
    ...
    try {
      const res = await fetch("/api/trigger", {
        ...
        body: JSON.stringify({
          ...
          valuation_preset: valuationPreset,
        }),
      });
      if (res.status === 403) {
        setSubmitError("Invalid API key.");
        setSubmitting(false);
        return;
      }
      if (!res.ok) {
        setSubmitError("Failed to start screen.");
        setSubmitting(false);
        return;
      }
      onStarted();
      ...
      >
        ...
        <div className="px-5 py-5 flex flex-col gap-4 overflow-hidden min-h-0">
          ...
          <div className="shrink-0">
            <label
              className="block text-xs font-mono font-semibold uppercase tracking-widest mb-1.5"
              style={{ color: "var(--muted)" }}
            >
              API Key
            </label>
            ...
            >
              <span className="flex items-center gap-2 min-w-0">
                ...
                <span className="flex gap-1 shrink-0">
                  {AVAILABLE_MODELS.find((m) => m.id === model)?.tags.map(
                    (tag) => (
                      <span
                        key={tag}
              ...
              >
                {AVAILABLE_MODELS.map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="font-semibold w-36 shrink-0 truncate"
                        style={{
          ...
          <div className="flex flex-col min-h-0 flex-1">
            <label
              className="block text-xs font-mono font-semibold uppercase tracking-widest mb-1.5 shrink-0"
              style={{ color: "var(--muted)" }}
            >
              Tickers
            </label>
            ...
            >
              {count === 0
                ? "No tickers selected."
                : `${count} ticker${count === 1 ? "" : "s"} selected.`}
            </p>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1 shrink-0">
            ...
            >
              {submitting
                ? "Starting…"
                : `Run ${count > 0 ? `${count} ` : ""}${count === 1 ? "Ticker" : "Tickers"}`}
            </button>
          </div>
        </div>
      </div>
