...
const points: [number, number][] = [
  [0, 55], [30, 48], [60, 38], [90, 42], [120, 30],
  [150, 35], [180, 20], [210, 28], [240, 10], [270, 32],
  [300, 80],
];
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <div
        ...
        style={{ maxWidth: 360, borderColor: "var(--border)", backgroundColor: "var(--surface)" }}
      >
        {/* Card header */}
        <div className="px-5 pt-5 pb-4 text-left">
          <div className="flex items-start justify-between mb-1">
            <div>
              <span className="font-mono font-bold text-2xl tracking-tight" style={{ color: "var(--reject)" }}>
                ERRR
              </span>
              <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>Page Not Found Corp. · NYSE</p>
            </div>
            <span
          ...
          <div className="flex items-baseline gap-2 mt-3">
            <span className="font-mono text-3xl font-bold tabular-nums" style={{ color: "var(--text)" }}>$0.00</span>
            <span className="font-mono text-sm font-semibold" style={{ color: "var(--reject)" }}>−100.00%</span>
          </div>
        </div>

        {/* Flatline chart */}
        <div style={{ backgroundColor: "color-mix(in srgb, var(--reject) 4%, var(--surface))" }}>
          <svg
            width="100%"
          ...
          >
            <defs>
              <linearGradient id="crash-fill" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="var(--reject)" stopOpacity="0.15" />
                <stop offset="100%" stopColor="var(--reject)" stopOpacity="0" />
              </linearGradient>
            </defs>
            <path d={toArea(points)} fill="url(#crash-fill)" />
            <path d={toPath(points)} fill="none" stroke="var(--reject)" strokeWidth="1.5" strokeLinejoin="round" />
          </svg>
        </div>
        ...
        >
          {[
            ...
            <div key={label} className="flex flex-col items-center py-1">
              ...
              <span className="font-mono text-sm font-semibold tabular-nums mt-0.5" style={{ color: "var(--text)" }}>{value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <p className="text-sm mb-8" style={{ color: "var(--muted)", maxWidth: 280 }}>
